...
1. **批处理** — Items are scored in batches of 10 with a progress bar. Failed items receive a score of 0.
2. **内容编制** — For each item, the content is truncated (800 chars if comments are present, 1000 otherwise) and engagement metrics are assembled from metadata (HN score, Reddit upvote ratio, etc.).
3. **AI分析** — The prepared content is sent to the configured AI model (temperature 0.3) with a system prompt defining the scoring criteria.
4. **反应解析** — The AI response is parsed as JSON (with fallbacks for code-block-wrapped JSON). Each item gets: `ai_score` (float), `ai_reason` (string), `ai_summary` (string), and `ai_tags` (list).
...
- **撰写/陈述的质量** — clarity, structure, thoroughness
- **社区讨论** — insightful comments, diverse viewpoints, substantive debates
- **交战信号** — high upvotes/favorites paired with substantive discussion (not just raw numbers)
...
2. **网络搜索** — Each concept is searched via DuckDuckGo to gather grounding context.
3. **结构分析** — The item content and search results are sent to AI, which produces:
