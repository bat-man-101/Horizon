---
layout: default
title: "Horizon Summary: 2026-09-10 (ZH)"
date: 2026-09-10
lang: zh
---

> 从 1609 条内容中筛选出 15 条重要资讯。

---

**📌 其他（3）**
  1. [iPhone 双核](#item-1) ⭐️ ?/10
  2. [Shopify 收购 Tailwind](#item-2) ⭐️ ?/10
  3. [AirPods 5](#item-3) ⭐️ ?/10

**🤖 AI 新闻（3）**
  4. [引用加州研究](#item-4) ⭐️ ?/10
  5. [.blend URL 查看器](#item-5) ⭐️ ?/10
  6. [人工智能政策窗口已打开。我们需要采取行动。](#item-6) ⭐️ ?/10

**🔬 半导体（1）**
  7. [机器人在哪里思考——设备上推理与数据中心推理](#item-7) ⭐️ ?/10

**₿ 加密资产（1）**
  8. [Robinhood 首席执行官 Vlad Tenev 回击 AMC 的 Aron，因股票代币之争不断升级](#item-8) ⭐️ ?/10

**🚀 科技动态（3）**
  9. [人工智能研究初创公司 Listen Labs 取消了 Salesforce 谈判的 1.5 亿美元融资](#item-9) ⭐️ ?/10
  10. [Automattic 董事会迫使首席执行官 Matt Mullenweg 休假](#item-10) ⭐️ ?/10
  11. [OpenAI 董事会增加了一位杰出的 AI 毁灭者](#item-11) ⭐️ ?/10

**📄 论文研究（2）**
  12. [超越正确与错误：评估大型语言模型中的二阶社会推理](#item-12) ⭐️ ?/10
  13. [CriticGen：世代感知评估作为可行的反馈](#item-13) ⭐️ ?/10

**📰 热点新闻（2）**
  14. [监管机构表示，英国需要针对医疗保健领域的人工智能制定新法律](#item-14) ⭐️ ?/10
  15. [谷歌选择芬兰作为其在欧洲最大的单一投资](#item-15) ⭐️ ?/10
---

## 📌 其他

<a id="item-1"></a>
## [iPhone 双核](https://www.apple.com/iphone-duo/) ⭐️ ?/10



hackernews · thecosmicfrog · 9月9日 18:15 · [社区讨论](https://news.ycombinator.com/item?id=49630931)

---

<a id="item-2"></a>
## [Shopify 收购 Tailwind](https://tailwindcss.com/blog/tailwind-is-joining-shopify) ⭐️ ?/10



hackernews · EdwinHoksberg · 9月9日 13:27 · [社区讨论](https://news.ycombinator.com/item?id=49626190)

---

<a id="item-3"></a>
## [AirPods 5](https://www.apple.com/newsroom/2026/09/apple-introduces-airpods-5-with-best-in-class-open-ear-active-noise-cancellation/) ⭐️ ?/10



hackernews · awad · 9月9日 17:39 · [社区讨论](https://news.ycombinator.com/item?id=49630253)

---

## 🤖 AI 新闻

<a id="item-4"></a>
## [引用加州研究](https://simonwillison.net/2026/Sep/10/calif-research/) ⭐️ ?/10

Today, we're releasing a demo of WeWorm, the first zero-click worm to spread through WeChat calls across iOS and Android. [...]
The victim does not need to answer the call, or interact with their phone at all. Even if they do answer, they hear nothing, and the exploit still succeeds. [...]
Working with AI, our team found the bug and wrote the first remote code execution (RCE) exploit in about two days. Building the worm took one more week.
A worm at this scale used to be the kind of thing that t

rss · Simon Willison · 9月10日 00:56

---

<a id="item-5"></a>
## [.blend URL 查看器](https://simonwillison.net/2026/Sep/9/blender-viewer/) ⭐️ ?/10

Tool: .blend URL Viewer
        I'm continuing to have a lot of fun with GPT-6 Astra and Blender (see my TIL).
As a big fan of the Imperial Fabergé Easter eggs, I've always thought it would be fun to make some new ones that celebrate popular culture.
Yesterday I decided to try out the new ChatGPT Images 2.5 by running this prompt:

Generate a photo of a faberge egg that's themed after the TV show Pluribus - research first

It gave me this - honestly not bad for a first attempt!

Then, just to se

rss · Simon Willison · 9月9日 23:58

---

<a id="item-6"></a>
## 【人工智能政策窗口已打开。我们需要采取行动。](https://openai.com/index/ai-policy-window) ⭐️ ?/10

Chris Lehane argues that stronger AI capabilities require stronger safety evidence, shared standards, and durable policy action while the policy window remains open.

rss · OpenAI Blog · 9月9日 13:00

---

## 🔬 半导体

<a id="item-7"></a>
## [机器人在哪里思考 – 设备上推理与数据中心推理](https://newsletter.semianalysis.com/p/where-does-a-robot-think-on-device) ⭐️ ?/10

For most of its short history, AI lived behind a screen.

rss · Semianalysis · 9月9日 20:53

---

## ₿ 加密资产

<a id="item-8"></a>
## [Robinhood 首席执行官 Vlad Tenev 回击 AMC 的 Aron，因股票代币之争不断升级](https://www.coindesk.com/markets/2026/09/09/robinhood-s-ceo-vlad-tenev-defends-stock-tokens-in-escalating-fight-with-amc-ceo) ⭐️ ?/10



rss · CoinDesk · 9月9日 16:11

---

## 🚀 科技动态

<a id="item-9"></a>
## [人工智能研究初创公司 Listen Labs 取消了 Salesforce 会谈的 1.5 亿美元融资](https://techcrunch.com/2026/09/09/ai-research-startup-listen-labs-scrubbed-a-1-5b-funding-round-for-salesforce-talks/) ⭐️ ?/10

Listen Labs walked away from a signed Series C term sheet from Menlo Ventures, sources say.

rss · 36氪 - 科技 · 9月10日 00:00

---

<a id="item-10"></a>
## [Automattic 董事会迫使首席执行官 Matt Mullenweg 请假](https://techcrunch.com/2026/09/09/automattics-board-forces-ceo-matt-mullenweg-into-leave-of-absence/) ⭐️ ?/10

Mullenweg said in a company Slack message that it was against his will.

rss · 36氪 - 科技 · 9月9日 23:14

---

<a id="item-11"></a>
## [OpenAI 在其董事会中增加了一位杰出的 AI 末日者](https://techcrunch.com/2026/09/09/openai-adds-a-prominent-ai-doomer-to-its-board-of-directors/) ⭐️ ?/10

Paul Christiano, an influential AI researcher focused on alignment, is joining the OpenAI Foundation as a member of its board.

rss · 36氪 - 科技 · 9月9日 22:25

---

## 📄 论文研究

<a id="item-12"></a>
## [超越正确与错误：评估大型语言模型中的二阶社会推理](https://arxiv.org/abs/2609.05437) ⭐️ ?/10

arXiv:2609.05437v1 Announce Type: new 
Abstract: Previous AI alignment efforts have focused primarily on first-order social norms -- teaching models what is socially acceptable or unacceptable (e.g., `do not steal'). However, social intelligence depends not only on norm recognition, but also on anticipating who will enforce it and how (e.g., public shame or even imprisonment). These second-order expectations, known as metanorms, govern how people respond when social rules are broken. We introduc

rss · arXiv AI · 9月9日 04:00

---

<a id="item-13"></a>
## [CriticGen：世代感知评估作为可行的反馈](https://arxiv.org/abs/2609.05439) ⭐️ ?/10

arXiv:2609.05439v1 Announce Type: new 
Abstract: Current evaluation methods for large language models are coarse-grained and decoupled from generation, producing generic explanations that fail to provide actionable feedback for model improvement. We propose CriticGen, a fine-grained, generation-aware evaluation framework that turns evaluation into actionable control for answer improvement. CriticGen first generates sample-specific evaluation dimensions and scoring criteria under high-level categ

rss · arXiv AI · 9月9日 04:00

---

## 📰 热点新闻

<a id="item-14"></a>
## [看门狗表示，英国需要针对医疗保健领域的人工智能制定新法律](https://www.bbc.co.uk/news/articles/c3wjn3pl63xo?at_medium=RSS&at_campaign=rss) ⭐️ ?/10

The technology will soon be routinely used within the NHS, MHRA chief Lawrence Tallon tells the BBC.

rss · BBC Technology · 9月10日 00:48

---

<a id="item-15"></a>
## [谷歌选择芬兰作为其在欧洲最大的单一投资](https://www.bbc.co.uk/news/articles/c8r6y4me2g6o?at_medium=RSS&at_campaign=rss) ⭐️ ?/10

The US tech giant says the €13bn data centre project will create tens of thousands of jobs.

rss · BBC Technology · 9月9日 17:06

---